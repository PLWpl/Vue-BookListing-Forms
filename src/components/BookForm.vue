<template>
  <form v-on:submit.prevent="bookSubmit(bookData)" class="" action="#" method="post">
    ...
    <div>
      ...
      <input name="finishedReading" type="checkbox" v-model="bookData.finishedReading"></label>
    </div>
    <div>
      <label for="borrowed">borrowed
      <input name="borrowed" type="radio" v-model="bookData.ownership" value="borrowed"></label>
      <label for="bought">bought
      <input name="bought" type="radio" v-model="bookData.ownership" value="bought"></label>
    </div>
